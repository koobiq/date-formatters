...
describe('MomentDateAdapter', () => {
    ...
    it('should calculate startOf properly', () => {
        ...
        const now = moment.utc(adapter.createDateTime(2011, 11, 11, 11, 11, 11, 11));

        expect(dateUnits.map((unit) => adapter.toIso8601(adapter.startOf(now.clone(), unit)))).toMatchSnapshot();
    });
});
...
describe('MomentDateAdapter findDateFormat = true', () => {
    ...
    describe('addCalendarUnits', () => {
        ...
        beforeEach(() => {
            ...
            baseDate = moment('2024-01-15T10:30:00.000Z');
        });

        describe('when called with number and unit', () => {
            it('should add years correctly', () => {
                const original = baseDate.clone();
                const result = adapter.addCalendarUnits(original, 2, 'years');
                expect(result.year()).toBe(2026);
                expect(result.month()).toBe(baseDate.month());
                expect(result.date()).toBe(baseDate.date());
            });

            it('should add months correctly', () => {
                const original = baseDate.clone();
                const result = adapter.addCalendarUnits(original, 3, 'months');
                expect(result.year()).toBe(2024);
                expect(result.month()).toBe(3); // 0-indexed, so 3 = April
                expect(result.date()).toBe(baseDate.date());
            });

            it('should add weeks correctly', () => {
                const original = baseDate.clone();
                const result = adapter.addCalendarUnits(original, 2, 'weeks');
                expect(result.date()).toBe(29);
            });

            it('should add days correctly', () => {
                const original = baseDate.clone();
                const result = adapter.addCalendarUnits(original, 5, 'days');
                expect(result.date()).toBe(20);
            });

            it('should add hours correctly', () => {
                const original = baseDate.clone();
                const result = adapter.addCalendarUnits(original, 3, 'hours');
                expect(result.toISOString()).toMatchSnapshot();
            });

            it('should add minutes correctly', () => {
                const original = baseDate.clone();
                const result = adapter.addCalendarUnits(original, 45, 'minutes');
                expect(result.minute()).toBe(15);
            });

            it('should add seconds correctly', () => {
                const original = baseDate.clone();
                const result = adapter.addCalendarUnits(original, 30, 'seconds');
                expect(result.second()).toBe(30);
            });

            it('should add milliseconds correctly', () => {
                const original = baseDate.clone();
                const result = adapter.addCalendarUnits(original, 500, 'milliseconds');
                expect(result.millisecond()).toBe(500);
            });

            it('should handle negative values', () => {
                const original = baseDate.clone();
                const result = adapter.addCalendarUnits(original, -1, 'months');
            ...
            it('should handle zero values', () => {
                const original = baseDate.clone();
                const result = adapter.addCalendarUnits(original, 0, 'days');
            ...
            it('should return original date when only number is provided without unit', () => {
                const original = baseDate.clone();
                const result = adapter.addCalendarUnits(original, 5);
                expect(result.toISOString()).toBe(original.toISOString());
            });

            it('should immutable by default', () => {
                const cloned = adapter.addCalendarUnits(baseDate, 5, 'days');
                // Original remains unchanged
                expect(baseDate.date()).toBe(15);
                expect(cloned.date()).toBe(20);
            });

            it('should handle invalid dates gracefully', () => {
                const invalidDate = moment('invalid');
                const result = adapter.addCalendarUnits(invalidDate, 1, 'days');
        ...
        describe('when called with DurationObjectUnits object', () => {
            it('should add multiple units from object', () => {
                ...
                const result = adapter.addCalendarUnits(original, duration);
                expect(result.year()).toBe(2025);
                expect(result.month()).toBe(2); // 0-indexed, so 2 = March
                expect(result.date()).toBe(18);
            });

            it('should handle object with single unit', () => {
                const original = baseDate.clone();
                const duration: DurationObjectUnits = { hours: 5 };
                const result = adapter.addCalendarUnits(original, duration);
                expect(result.toISOString()).toMatchSnapshot();
            });
            ...
            it('should handle quarters if supported', () => {
                const original = baseDate.clone();
                const duration: DurationObjectUnits = { quarters: 2 };
                const result = adapter.addCalendarUnits(original, duration);
                expect(result.month()).toBe(6); // 0-indexed, so 6 = July (6 months later)
            });
        });
    });
});
