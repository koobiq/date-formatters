import {
    ...
    DateUnit,
    DurationUnit,
    DurationObjectUnits as ObjectUnits
} from '@koobiq/date-adapter';
import { DateTime, DateTimeOptions, DateTimeUnit, Duration, DurationObjectUnits, Interval, LocaleOptions } from 'luxon';
...
export class LuxonDateAdapter extends DateAdapter<DateTime> {
    ...
    format(date: DateTime, displayFormat: string): string {
        ...
        if (this.locale === ruRU.name && displayFormat?.search(/([^M]|^)MMM([^M]|$)/) !== -1) {
            return this.formatShortMonthForRULocale(reconfiguredDate, displayFormat, this.dateTimeOptions);
        } else if (this.locale === ptBR.name) {
            return this.formatShortMonthForPTLocale(reconfiguredDate, displayFormat, this.dateTimeOptions);
        } else if (this.locale === tkTM.name && displayFormat?.includes('MMM')) {
            return this.formatMonthForTKLocale(reconfiguredDate, displayFormat, this.dateTimeOptions);
        }

        return reconfiguredDate.toFormat(displayFormat, this.dateTimeOptions);
    }

    addCalendarUnits(date: DateTime, amountOrDurationLikeObject: number | ObjectUnits, unit?: DurationUnit): DateTime {
        if (typeof amountOrDurationLikeObject === 'number' && unit !== undefined) {
            return date.plus({ [unit]: amountOrDurationLikeObject });
        }

        if (typeof amountOrDurationLikeObject === 'object') {
            return date.plus(amountOrDurationLikeObject);
        }
    ...
    startOf(date: DateTime, unit: DateUnit): DateTime {
        ...
        return this.reconfigure(date.startOf(unit));
    }
    ...
    // we need to do this strange things because Intl has ugly reduction for russian months
    private formatMonthForTKLocale(originalDate: DateTime, displayFormat: string, options: LocaleOptions) {
        const date = originalDate.reconfigure({ ...options, locale: 'en-US' });
        const formatted = date.toFormat(displayFormat, options);

        if (displayFormat.includes('MMMM')) {
            return formatted.replace(date.monthLong as string, this.config.monthNames.long[date.month - 1]);
        } else if (displayFormat.includes('MMM')) {
            return formatted.replace(date.monthShort as string, this.config.monthNames.short.formatted[date.month - 1]);
        }

        throw new Error(`Unknown displayFormat: ${displayFormat}`);
    }
