...
describe('LuxonDateAdapter', () => {
    ...
    it('should calculate startOf properly', () => {
        ...
        const now = adapter.createDateTime(2011, 11, 11, 11, 11, 11, 11).toUTC();
    ...
    describe('addCalendarUnits', () => {
        ...
        beforeEach(() => {
            ...
            baseDate = DateTime.fromISO('2024-01-15T10:30:00.000Z').toUTC();
        });
        ...
        describe('when called with number and unit', () => {
            it('should add years correctly', () => {
                ...
                expect(result.day).toBe(baseDate.day);
            });
            ...
            it('should add weeks correctly', () => {
                const result = adapter.addCalendarUnits(baseDate, 2, 'weeks');
                expect(result.day).toBe(29);
            });

            it('should add days correctly', () => {
                const result = adapter.addCalendarUnits(baseDate, 5, 'days');
                expect(result.day).toBe(20);
            });

            it('should add hours correctly', () => {
                const result = adapter.addCalendarUnits(baseDate, 3, 'hours').toUTC();
                expect(result.toISO()).toMatchSnapshot();
            });

            it('should add minutes correctly', () => {
                const result = adapter.addCalendarUnits(baseDate, 45, 'minutes');
                expect(result.minute).toBe(15);
            });

            it('should add seconds correctly', () => {
                const result = adapter.addCalendarUnits(baseDate, 30, 'seconds');
                expect(result.second).toBe(30);
            });

            it('should add milliseconds correctly', () => {
                const result = adapter.addCalendarUnits(baseDate, 500, 'milliseconds');
            ...
            it('should handle zero values', () => {
                const result = adapter.addCalendarUnits(baseDate, 0, 'days');
                expect(result.toISO()).toBe(baseDate.toISO());
            });

            it('should return original date when only number is provided without unit', () => {
                const result = adapter.addCalendarUnits(baseDate, 5);
                expect(result.toISO()).toBe(baseDate.toISO());
            });
        });
